...
//定义一个菜单的block
typedef void(^FFMenuBlock)(NSInteger index);
 ...
 */
+ (instancetype)ff_DefaultStyleDropDownMenuWithMenuModelsArray:(NSArray *)menuModelsArray menuWidth:(CGFloat)menuWidth eachItemHeight:(CGFloat)eachItemHeight menuRightMargin:(CGFloat)menuRightMargin triangleRightMargin:(CGFloat)triangleRightMargin menuBlock:(FFMenuBlock _Nullable)menuBlock;
